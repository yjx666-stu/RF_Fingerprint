...
CAPTURE_DIR="/home/elf/rf_fingerprint/live_capture/video_demo_device0_attack_session_auto"
OUT_DIR="/home/elf/rf_fingerprint/live_auth_monitor/video_demo_device0_attack_session_auto"

echo "============================================================"
echo "LoRa RF Fingerprint Demo: device0 impersonates device1"
echo "Mode: event trigger + session stop + evidence saving"
echo "============================================================"
...
echo "[2/5] Start IQ capture..."
echo "Please make sure: device0 ON, device1 OFF"
...
echo "[CHECK] Manifest:"
cat "$CAPTURE_DIR/manifest.csv"

echo "[3/5] Capture finished. Start event-triggered authentication..."
python3 /home/elf/rf_fingerprint/scripts/trigger_auth_monitor.py \
  ...
  --stop_after_attack_segments 1 \
  --stop_after_mismatch_segments 1 \
...
result_path = Path("$OUT_DIR/final_verify_claimed_device.json")
...
AUTH_RESULT = str(result.get("final_result", "") or "")
CLAIMED_DEVICE = str(result.get("claimed_device", "") or summary.get("claimed_device", "") or "device1")
RFFI_RESULT = (
    str(result.get("dominant_vote_pred", "") or "").strip()
    or str(summary.get("dominant_vote_pred", "") or summary.get("dominant_vote", "") or "").strip()
...
echo "Final verify JSON:"
echo "$OUT_DIR/final_verify_claimed_device.json"
echo "============================================================"
